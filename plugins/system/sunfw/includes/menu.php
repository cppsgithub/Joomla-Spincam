...
class SunFwMenu
{
	...
	*/
	...
	{
		$rand = strtolower(SunFwUltils::generateRandString());
		...
		if ( $mobile_target == 1 )
		{
			return '<nav class="navbar navbar-default sunfw-menu-head'. $classsubeffect .'" role="navigation">
				<div class="navbar-header">
					<button type="button" class="navbar-toggle collapsed btn btn-danger" data-toggle="collapse" data-target="#menu_'. $id .'" aria-expanded="false">
						<i aria-hidden="true" class="fa fa-bars"></i>
					</button>
				</div>
				<div class="collapse navbar-collapse sunfw-menu" id="menu_'. $id .'"><ul class="nav navbar-nav sunfw-tpl-menu">';
		}
		else
		{
	...
	public static function renderComponentItemLayout($item, $activeID, $showicon = 0, $showdescription = 0, $megamenu = '')
	{
		...
		switch ($item->browserNav)
		{
			default:
			case 0:
				$html = '<a ' . $class . 'href="' . $item->flink . '" ' . $title . '>' . $icon . $linktype . $caret .'</a>';
			break;

			case 1:
				$html = '<a ' . $class . 'href="' . $item->flink . '" target="_blank" rel="noopener noreferrer" ' . $title . '>' . $icon . $linktype . $caret .'</a>';
			break;

			case 2:
				$html = '<a ' . $class . 'href="' . $item->flink . '" onclick="window.open(this.href,\'targetWindow\',\'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes\');return false;" ' . $title . '>' . $icon . $linktype . $caret . '</a>';
			break;
		}
	...
	public static function renderUrlItemLayout($item, $activeID, $showicon = 0, $showdescription = 0, $megamenu = '')
	{
		...
		$class_icon = $item->params->get('sunfw-link-icon', '');
		$title = $item->anchor_title ? 'title="' . $item->anchor_title . '" ' : '';
		$link_icon = $item->params->get('sunfw-link-icon', '');
		$link_description = $item->params->get('sunfw-link-description', '');
		...
		switch ($item->browserNav)
		{
			default:
			case 0:
				$html .= '<a ' . $class . 'href="' . $flink . '" ' . $title . '>' . $icon . $linktype . $caret .'</a>';
			break;

			case 1:
				$html .= '<a ' . $class . 'href="' . $flink . '" target="_blank" rel="noopener noreferrer" ' . $title . '>' . $icon . $linktype . $caret .'</a>';
			break;

			case 2:
				$options = 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes';
				$html .= '<a ' . $class . 'href="' . $flink . '" onclick="window.open(this.href,\'targetWindow\',\'' . $options . '\');return false;" ' . $title . '>' . $icon . $linktype . $caret . '</a>';
			break;
		}
	 ...
	 * @return string
	 */
	public static function renderHeadingItemLayout($item, $activeID)
	{
		$html = '';
		$title = $item->anchor_title ? 'title="' . $item->anchor_title . '" ' : '';
	...
	public static function renderMegaMenu( $root )
	{
		...
		if (
			! @count( $megamenu )
			//||
			//! @isset( $megamenu['megamenu'][ $root ]['settings']['enabled'] )
			//||
			//! intval( $megamenu['megamenu'][ $root ]['settings']['enabled'] )
			||
			! @isset( $megamenu['megamenu'][ $root ]['rows'] )
			||
			! @count( $megamenu['megamenu'][ $root ]['rows'] )
		)
		{
			return;
		}
		...
		{
			if ( ! empty( $root['settings']['padding']['top'] ) )
			{
				$styles[] = "padding-top:{$root['settings']['padding']['top']}px";
			}

			if ( ! empty( $root['settings']['padding']['right'] ) )
			{
				$styles[] = "padding-right:{$root['settings']['padding']['right']}px";
			}

			if ( ! empty( $root['settings']['padding']['bottom'] ) )
			{
				$styles[] = "padding-bottom:{$root['settings']['padding']['bottom']}px";
			}

			if ( ! empty( $root['settings']['padding']['left'] ) )
			{
				$styles[] = "padding-left:{$root['settings']['padding']['left']}px";
			}
		}
		...
		if ( ! empty( $root['settings']['backgroundImage'] ) )
		{
			...
			{
				if ( ! empty( $root['settings']['backgroundImageSettings']['repeat'] ) )
				{
					$styles[] = "background-repeat:{$root['settings']['backgroundImageSettings']['repeat']}";
				}

				if ( ! empty( $root['settings']['backgroundImageSettings']['attachment'] ) )
				{
					$styles[] = "background-attachment:{$root['settings']['backgroundImageSettings']['attachment']}";
				}

				if ( ! empty( $root['settings']['backgroundImageSettings']['size'] ) )
				{
					$styles[] = "background-size:{$root['settings']['backgroundImageSettings']['size']}";
				}

				if ( ! empty( $root['settings']['backgroundImageSettings']['cover'] ) )
				{
					$styles[] = "background-cover:{$root['settings']['backgroundImageSettings']['cover']}";
				}

				if ( ! empty( $root['settings']['backgroundImageSettings']['position'] ) )
				{
					$styles[] = "background-position:{$root['settings']['backgroundImageSettings']['position']}";
				}
			}
		}
		...
		if ( @is_array( $root['settings']['border'] ) )
		{
			if ( isset( $root['settings']['border']['universal'] ) && intval( $root['settings']['border']['universal'] ) ) {
				if ( ! empty( $root['settings']['border']['width'] ) )
				{
					$styles[] = "border-width:{$root['settings']['border']['width']}px";
				}

				if ( ! empty( $root['settings']['border']['style'] ) )
				{
					$styles[] = "border-style:{$root['settings']['border']['style']}";
				}

				if ( ! empty( $root['settings']['border']['color'] ) )
				{
					$styles[] = "border-color:{$root['settings']['border']['color']}";
				}
			}
			else
			{
				...
				{
					$pwidth = $position . '-width';
					$pstyle = $position . '-style';
					$pcolor = $position . '-color';

					if ( ! empty( $root['settings']['border'][$pwidth] ) )
					{
						$styles[] = "border-{$position}-width:{$root['settings']['border'][$pwidth]}px";
					}

					if ( ! empty( $root['settings']['border'][$pstyle] ) )
					{
						$styles[] = "border-{$position}-style:{$root['settings']['border'][$pstyle]}";
					}

					if ( ! empty( $root['settings']['border'][$pcolor] ) )
					{
						$styles[] = "border-{$position}-color:{$root['settings']['border'][$pcolor]}";
					}
				}
			}
		}
		...
		foreach ( $root['rows'] as $rowIndex )
		{
			...
			if (
				...
				! @count( $row['columns'] )
				||
				( @isset( $row['settings']['disabled'] ) && intval( $row['settings']['disabled'] ) )
			)
			{
				continue;
			}
			...
			{
				if ( ! empty( $row['settings']['padding']['top'] ) ) {
					$scripts[] = '$("#'.$rowID.'").css("padding-top", "'.$row['settings']['padding']['top'].'px");';
				}

				if ( ! empty( $row['settings']['padding']['right'] ) ) {
					$scripts[] = '$("#'.$rowID.'").css("padding-right", "'.$row['settings']['padding']['right'].'px");';
				}

				if ( ! empty( $row['settings']['padding']['bottom'] ) ) {
					$scripts[] = '$("#'.$rowID.'").css("padding-bottom", "'.$row['settings']['padding']['bottom'].'px");';
				}

				if ( ! empty( $row['settings']['padding']['left'] ) ) {
					$scripts[] = '$("#'.$rowID.'").css("padding-left", "'.$row['settings']['padding']['left'].'px");';
				}
			}

			if ( @is_array( $row['settings']['margin'] ) )
			{
				if ( ! empty( $row['settings']['margin']['top'] ) ) {
					$scripts[] = '$("#'.$rowID.'").css("margin-top", "'.$row['settings']['margin']['top'].'px");';
				}

				if ( ! empty( $row['settings']['margin']['right'] ) ) {
					$scripts[] = '$("#'.$rowID.'").css("margin-right", "'.$row['settings']['margin']['right'].'px");';
				}

				if ( ! empty( $row['settings']['margin']['bottom'] ) ) {
					$scripts[] = '$("#'.$rowID.'").css("margin-bottom", "'.$row['settings']['margin']['bottom'].'px");';
				}

				if ( ! empty( $row['settings']['margin']['left'] ) ) {
					$scripts[] = '$("#'.$rowID.'").css("margin-left", "'.$row['settings']['margin']['left'].'px");';
				}
			}

			// Loop thru columns to render HTML.
			$megaMenuHtml .= '<div id="sunfw_menu_'.$row['id'].'" class="row">';
			...
			foreach ( $row['columns'] as $columnIndex )
			{
				...
				{
					if ( ! empty( $column['settings']['padding']['top'] ) ) {
						$scripts[] = '$("#'.$columnID.'").css("padding-top", "'.$column['settings']['padding']['top'].'px");';
					}

					if ( ! empty( $column['settings']['padding']['right'] ) ) {
						$scripts[] = '$("#'.$columnID.'").css("padding-right", "'.$column['settings']['padding']['right'].'px");';
					}

					if ( ! empty( $column['settings']['padding']['bottom'] ) ) {
						$scripts[] = '$("#'.$columnID.'").css("padding-bottom", "'.$column['settings']['padding']['bottom'].'px");';
					}

					if ( ! empty( $column['settings']['padding']['left'] ) ) {
						$scripts[] = '$("#'.$columnID.'").css("padding-left", "'.$column['settings']['padding']['left'].'px");';
					}
				}

				if ( @is_array( $column['settings']['margin'] ) )
				{
					if ( ! empty( $column['settings']['margin']['top'] ) ) {
						$scripts[] = '$("#'.$columnID.'").css("margin-top", "'.$column['settings']['margin']['top'].'px");';
					}

					if ( ! empty( $column['settings']['margin']['right'] ) ) {
						$scripts[] = '$("#'.$columnID.'").css("margin-right", "'.$column['settings']['margin']['right'].'px");';
					}

					if ( ! empty( $column['settings']['margin']['bottom'] ) ) {
						$scripts[] = '$("#'.$columnID.'").css("margin-bottom", "'.$column['settings']['margin']['bottom'].'px");';
					}

					if ( ! empty( $column['settings']['margin']['left'] ) ) {
						$scripts[] = '$("#'.$columnID.'").css("margin-left", "'.$column['settings']['margin']['left'].'px");';
					}
				}

				// Loop thru blocks to render HTML.
				$megaMenuHtml .= '<div id="sunfw_menu_'.$column['id'].'" class="col-xs-' . $column['width'] .(@isset($column['settings']['class']) ? ' '. $column['settings']['class'] : '') . '">';
				...
				foreach ( $column['blocks'] as $blockIndex )
				{
					...
					$megaMenuHtml .= '<div class="sunfw-block">';
					...
					foreach ( $block['items'] as $itemIndex )
					{
						...
						$megaMenuHtml .= '<div class="sunfw-item">';
