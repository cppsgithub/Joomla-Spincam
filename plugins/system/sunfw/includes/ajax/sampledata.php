...
class SunFwAjaxSampleData extends SunFwAjax
{
    ...
    public function getAction()
    {
		...
		foreach (glob( JPATH_SITE . "/templates/{$this->templateName}/niches/*" ) as $niche)
		{
			if ( ! is_dir($niche) )
			{
				continue;
			...
			$niches[] = array(
				'id'        => strtolower($niche),
				'name'      => ucfirst($niche),
				'thumbnail' => "{$root}/plugins/system/sunfw/assets/images/no-image.png",
				'demo'      => '#',
				'download'  => '#',
				'tags'      => array()
			);
		...
		if ( count($niches) )
		{
			foreach ($niches as $k => $niche)
			{
				foreach ($packages as $package)
				{
					if ($niche['id'] == $package['id'])
					{
						unset($niches[$k]);
					}
				}
		...
		if ( count($niches) )
		{
			$packages = array_merge($packages, $niches);
		}
        ...
        try
        {
            ...
            else
            {
                $packageFile = SunFwApiLightcart::downloadPackage(
                    $id, 'FREE', null, null, $config->get( 'tmp_path' ) . "/{$this->template->id}/", $silent
                );
            }
            ...
            {
                throw new Exception(
                    JText::sprintf( 'SUNFW_FAILED_TO_DOWNLOAD_EXTENSION_PACKAGE', ( string ) $extension['description'] )
                );
            }
            ...
            foreach ( $this->xml->xpath( '//extension' ) as $extension )
            {
                ...
                if ( $canInstall )
                {
                    ...
                    foreach ( $queries as $query )
                    {
						...
						{
							$tmpQuery = explode( '<base href=', (string) $query );
							$query = $tmpQuery[0];

							for ( $i = 1, $n = count($tmpQuery); $i < $n; $i++ )
							{
								$tmpQuery[$i] = explode( ' />', $tmpQuery[$i], 2 );
								$query .= array_pop( $tmpQuery[$i] );
							}
						}
