 ...
 * This uses the {{{pb2loadmodule id}}} syntax.
 */
class PlgContentPB2LoadModule extends JPlugin
{
	/**
	...
	public function onContentPrepare( $context, &$article, &$params, $page = 0 )
	{
		...
		if ( strpos( $article->text, 'pb2loadmodule') === false )
		{
			return true;
		}

		// Expression to search for module to load.
		$regex = '/{{{pb2loadmodule\s(.*?)}}}/i';
		$regex2 = '/{pb2loadmodule\s(.*?)}/i';
		// Find all instances of plugin and put in $matches.
		if ( preg_match_all( $regex, $article->text, $matches, PREG_SET_ORDER ) || preg_match_all( $regex2, $article->text, $matches, PREG_SET_ORDER ) )
		{
