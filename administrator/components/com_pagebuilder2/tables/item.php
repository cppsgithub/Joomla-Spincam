 ...
 * @since    1.0.0
 */
class JSNPageBuilder2TableItem extends JTable
{
	/**
	...
	{
		parent::__construct('#__jsn_pagebuilder2_items', 'item_id', $db);
	}
	...
	public function loadFromLinkSource( $link_type, $link_id )
	{
		...
		$query->select( '*' )->from( '#__jsn_pagebuilder2_items' )->where( 'link_type = ' . $query->quote( $link_type ) . ' AND link_id = ' . ( int ) $link_id );
