...
{
	if ( ! defined('JSN_PAGEBUILDER2_DEPENDENCY'))
	{
	...
	{
		$check = json_decode(JSN_PAGEBUILDER2_DEPENDENCY);
	}
	...
	$model = new JSNPageBuilder2ModelInstaller;
	...
	if ( ! JSNVersion::isJoomlaCompatible(JSN_PAGEBUILDER2_REQUIRED_JOOMLA_VER))
	{
		try
		{
			$result = JSNUpdateHelper::check(JSN_PAGEBUILDER2_IDENTIFIED_NAME, JSN_PAGEBUILDER2_REQUIRED_JOOMLA_VER);

			if ($result[JSN_PAGEBUILDER2_IDENTIFIED_NAME])
			{
				$app->redirect('index.php?option=' . $input->getCmd('option') . '&view=update');
			}
			else
			{
				$app->enqueueMessage(JText::_('JSN_PAGEBUILDER2_NOT_COMPATIBLE_MSG'));
			}
		}
		catch (Exception $e)
		{
	...
	if ( ! JSNVersion::checkCompatibility(JSN_PAGEBUILDER2_IDENTIFIED_NAME, JSN_PAGEBUILDER2_VERSION))
	{
		$app->redirect('index.php?option=' . $input->getCmd('option') . '&view=update');
	}
}
