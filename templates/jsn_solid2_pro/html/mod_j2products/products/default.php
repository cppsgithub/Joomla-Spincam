...
?>
<div itemscope itemtype="http://schema.org/ItemList" class="j2store-product-module j2store-product-module-list row">
<?php if( count($list) > 0 ):?>
<?php foreach ($list as $product_id => $product) : ?>
	<?php  $rowcount = ((int) $counter % (int) $total_cols) + 1; ?> 
	<!-- single product -->
	<div class="j2store product-<?php echo $product->j2store_product_id; ?> j2store-module-product col-sm-6 col-md-<?php echo round((12 / $total_cols));?> custom">
		...
		<?php if($product->show_title): ?>
			<h4 itemprop="name" class="product-title">
				<?php if( $product->link_title ): ?>		
					<a itemprop="url" 
					href="<?php echo JRoute::_( $product->module_display_link ).'&task=view'; ?>" 
		...
		<div class="product_cart_block">
			...
			<?php JHTML::_('behavior.modal', 'a.modal'); ?>
				<a itemprop="url" style="display:inline;"
					...
					<i class="fa fa-eye"></i><span><?php echo JText::_('J2STORE_PRODUCT_QUICKVIEW');?></span>
				</a>
			<?php endif;?>
		</div>
	</div>
