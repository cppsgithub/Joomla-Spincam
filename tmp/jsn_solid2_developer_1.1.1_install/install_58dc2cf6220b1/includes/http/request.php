...
class_exists('http_class') OR require_once SUNFW_PATH . '/libraries/3rd-party/httpclient/http.php';
 ...
 * @since       1.0.0
 */
abstract class SunFwHttpRequest
{
	/**
	...
	public static function get($uri, $target = '', $validateHeader = true, $options = array())
	{
		...
		if (($error = $http->Open($arguments)) == '')
		{
			if (($error = $http->SendRequest($arguments)) == '')
			{
				...
				{
					throw new Exception(JText::sprintf('SUNFW_HTTP_CONNECTION_ERROR', $error));
				}
				...
				if ($validateHeader)
				{
					foreach ($result['header'] AS $header => $value)
					{
						...
						{
							throw new Exception(JText::sprintf('SUNFW_HTTP_CONNECTION_ERROR', substr($header, strpos($header, ' '))));
						}
					}
				}
